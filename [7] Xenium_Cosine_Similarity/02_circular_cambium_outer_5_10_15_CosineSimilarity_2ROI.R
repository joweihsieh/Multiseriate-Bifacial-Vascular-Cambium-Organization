#### Run this after completing the 2-ROI splitting step in 9. Xenium_5domain_DEG_all_samples.R

#/home/woodydrylab/FileShare/20260121_Xenium/output-XETG00360__0079031__BIO2_TISSUE_1__20260115__224443/grid05um_out/subset_from_k10_c01_c06/kmeans_subset_k03_raw_out
#!/usr/bin/env Rscript
# Select a ring of anchor bins near the cambium, push them outward along the radial direction
# by 5/10/15 µm, and then compare the transcriptomic difference between each anchor bin
# and its outward-pushed position.
# Supports:
# 1) Single-sample mode: bin_metadata_with_cluster_raw.tsv
# 2) Two-stem mode within the same folder: bin_metadata_with_cluster_raw_stem1.tsv / stem2.tsv
#    For combined samples listed in two.xlsx, the script automatically splits them into stem1 / stem2
#    and analyzes each stem separately.
...
# 改成 two 版本
SUBSET_XLSX <- file.path(BASE_DIR, "subset_cambium_5um_k10_k3_two.xlsx")
...
restrict_by_bbox <- function(dt, ref_dt, pad_um) {
  xr <- range(ref_dt$x_center, na.rm = TRUE)
  yr <- range(ref_dt$y_center, na.rm = TRUE)
  dt[
    x_center >= xr[1] - pad_um & x_center <= xr[2] + pad_um &
      y_center >= yr[1] - pad_um & y_center <= yr[2] + pad_um
  ]
}
...
run_one_sample <- function(sample_row) {
  ...
  subset_meta_file_default <- file.path(workdir, "bin_metadata_with_cluster_raw.tsv")
  subset_meta_file_stem1 <- file.path(workdir, "bin_metadata_with_cluster_raw_stem1.tsv")
  subset_meta_file_stem2 <- file.path(workdir, "bin_metadata_with_cluster_raw_stem2.tsv")

  has_split_subset <- file.exists(subset_meta_file_stem1) || file.exists(subset_meta_file_stem2)

  stem_jobs <- list()

  if (has_split_subset) {
    if (file.exists(subset_meta_file_stem1)) {
      stem_jobs[[length(stem_jobs) + 1]] <- list(
        stem_id = "stem1",
        stem_idx = 1L,
        subset_meta_file = subset_meta_file_stem1,
        sample_label = if (length(split_labels) >= 1) split_labels[1] else paste0(sample_label_all, "_stem1")
      )
    }
    if (file.exists(subset_meta_file_stem2)) {
      stem_jobs[[length(stem_jobs) + 1]] <- list(
        stem_id = "stem2",
        stem_idx = 2L,
        subset_meta_file = subset_meta_file_stem2,
        sample_label = if (length(split_labels) >= 2) split_labels[2] else paste0(sample_label_all, "_stem2")
      )
    }
  } else {
    stem_jobs[[1]] <- list(
      stem_id = "combined",
      stem_idx = NA_integer_,
      subset_meta_file = subset_meta_file_default,
      sample_label = sample_label_all
    )
  }

  results_one_sample <- vector("list", length(stem_jobs))

  for (jj in seq_along(stem_jobs)) {
    job <- stem_jobs[[jj]]

    cat("\n============================================================\n")
    cat("Processing sample_id: ", sample_id_input, "\n", sep = "")
    cat("stem_id: ", job$stem_id, "\n", sep = "")
    cat("label: ", job$sample_label, "\n", sep = "")
    ...
    cat("subset metadata file: ", job$subset_meta_file, "\n", sep = "")

    full_meta_file <- pick_first_existing(c(
      if (!is.na(job$stem_idx)) file.path(sample_base, sprintf("bin_metadata_stem%d.tsv", job$stem_idx)),
      file.path(sample_base, "bin_metadata.tsv")
    ))

    counts_rds <- file.path(sample_base, "counts_bins_by_genes_sparse.rds")

    k10_meta_file <- pick_first_existing(c(
      if (!is.na(job$stem_idx)) file.path(sample_base, "kmeans_k10_raw_out", sprintf("bin_metadata_with_cluster_raw_stem%d.tsv", job$stem_idx)),
      file.path(sample_base, "kmeans_k10_raw_out", "bin_metadata_with_cluster_raw.tsv")
    ))

    if (!file.exists(job$subset_meta_file)) stop("Missing subset metadata: ", job$subset_meta_file)
    if (is.na(full_meta_file) || !file.exists(full_meta_file)) stop("Missing full metadata for: ", sample_id_input)
    if (!file.exists(counts_rds)) stop("Missing counts RDS: ", counts_rds)
    if (is.na(k10_meta_file) || !file.exists(k10_meta_file)) stop("Missing k10 metadata for: ", sample_id_input)

    out_dir <- if (job$stem_id == "combined") {
      file.path(workdir, "radial_push_with_sclerenchyma_auto_20260409")
    } else {
      file.path(workdir, paste0("radial_push_with_sclerenchyma_auto_20260409_", job$stem_id))
    }
    safe_mkdir(out_dir)
    ...
    # ------------------------------------------------------------
    df_subset0 <- fread(job$subset_meta_file)
    ...
    if (nrow(df_subset) == 0) {
      stop("No bins left after filtering KEEP_CLUSTERS in sample: ", sample_id_input, " | ", job$stem_id)
    }
    ...
    # 如果 full metadata 不是 stem-specific，就用目前 stem 的 bbox 限制候選範圍
    if (!grepl(paste0(job$stem_id, "\\.tsv$"), basename(full_meta_file)) && job$stem_id != "combined") {
      pad_um <- max(PUSH_STEPS_UM) + NN_MAX_DIST + 2 * GRID_UM
      df_full <- restrict_by_bbox(df_full, df_subset, pad_um = pad_um)
      cat(sprintf("Restricted full metadata by stem bbox (+%.1f um): n=%d\n", pad_um, nrow(df_full)))
    }
    ...
    if (nrow(df_subset_clean) == 0) {
      stop("No bins left after global trimming in sample: ", sample_id_input, " | ", job$stem_id)
    }
    ...
    # 若 k10 metadata 不是 stem-specific，也限制到目前 stem bbox
    if (!grepl(paste0(job$stem_id, "\\.tsv$"), basename(k10_meta_file)) && job$stem_id != "combined") {
      pad_um <- max(PUSH_STEPS_UM) + NN_MAX_DIST + 2 * GRID_UM
      df_k10 <- restrict_by_bbox(df_k10, df_subset, pad_um = pad_um)
      cat(sprintf("Restricted k10 metadata by stem bbox (+%.1f um): n=%d\n", pad_um, nrow(df_k10)))
    }

    df_k10_scl <- df_k10[cluster_raw == sclerenchyma_cluster]
    cat(sprintf("k10 sclerenchyma bins: %d\n", nrow(df_k10_scl)))

    bg_plot <- df_subset_clean[, .(x = x_center, y = y_center, category = "subset")]
    scl_plot <- df_k10_scl[, .(x = x_center, y = y_center, category = "sclerenchyma")]
    anchor_tile_plot <- anchor_plot[, .(x = anchor_xg, y = anchor_yg, category = "anchor")]
    push_tile_plot <- push_plot[, .(x = xg, y = yg, category = paste0("step_", step_um))]
  ...
  # 2. 設定「邊框」(color): 只有 step 給黑色，其餘設定透明(不畫框)
  ...
  p_grid_alltiles_black <- ggplot(all_tiles) +
    geom_tile(
      # 在 aes 中同時綁定 fill 和 color
      aes(x = x, y = y, fill = category, color = category),
      width = GRID_UM,
      height = GRID_UM,
      linewidth = 0.05  # 控制黑框的粗細 (若你的 ggplot2 版本較舊，請改成 size = 0.3)
    ) +
    scale_fill_manual(values = tile_fill_colors_blackwhite, drop = FALSE) +
    scale_color_manual(values = tile_border_colors, drop = FALSE, guide = "none") + # 隱藏邊框的圖例  
    theme_classic() +
    theme(
    axis.text = element_blank(),
    axis.ticks = element_blank(),
  ...
  ggsave(
    ...
    p_dist <- ggplot(dist_tbl, aes(x = factor(step_um), y = transcriptome_cosine_dist_raw, fill = factor(step_um))) +
  ...
  cat(sprintf("Saved: %s\n", file.path(out_dir, "transcriptome_cosine_distance_raw_by_step.png")))
    ...
    results_one_sample[[jj]] <- data.table(
      sample_id = sample_id_input,
      stem_id = job$stem_id,
  ...
  rbindlist(results_one_sample, use.names = TRUE, fill = TRUE)
}
