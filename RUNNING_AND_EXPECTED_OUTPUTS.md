...
### Detailed expected outputs for the Xenium–scRNA distance workflow

The first distance script writes the following core files under its configured `OUTDIR`:

```text
xenium_initials_assignment.tsv
xenium_group_bin_count.tsv
singlecell_ray_fusiform_organizer_gene_nonzero_mean.tsv
singlecell_filter_stats.tsv
filter_summary.tsv
observed_metrics.tsv
permuted_metrics.tsv
combined_metrics.tsv
observed_euclidean_boxplot.pdf
observed_euclidean_boxplot.png
matched_vs_controls_euclidean_boxplot.pdf
matched_vs_controls_euclidean_boxplot.png
xenium_sc_with_controls_nonzero_mean_euclidean_result.rds
```

It also writes per-sample files such as:

```text
<SAMPLE>__xenium_filter_stats.tsv
<SAMPLE>__xenium_groups_gene_nonzero_mean.tsv
```

The second statistical script writes, depending on the retained code blocks:

```text
observed_euclidean_summary_raw.tsv
observed_euclidean_boxplot_raw.pdf
observed_euclidean_boxplot_raw.png
observed_euclidean_outlier_summary.tsv
observed_euclidean_pairwise_ttest_no_outlier.tsv
```
