...
# settings
############################################################
OUTDIR <- "uni_layer_based_26x9_5um"
...
############################################################
# build layer-based Uni pattern
############################################################
make_uni_pattern_layer <- function(nx = NX,
                                   ny = NY,
  ...
  # define layer groups
  left_layers  <- 1:left_layer_n
  mid_layers   <- (left_layer_n + 1):(left_layer_n + mid_layer_n)
  right_layers <- (left_layer_n + mid_layer_n + 1):(left_layer_n + mid_layer_n + right_layer_n)

  layer_dt <- data.table(layer_id = 1:(nx / layer_width))

  # layer-level means:
  # left side: linear from deep blue toward center
  layer_dt[layer_id %in% left_layers,
           layer_mean := seq(from = -1, to = -0.2, length.out = length(left_layers))]

  # middle layer: center value
  layer_dt[layer_id %in% mid_layers,
           layer_mean := 0]

  # right side: linear from center toward deep yellow
...
plot_pattern_um <- function(dt, outfile) {
  ...
  p <- ggplot(dt, aes(x = x_um, y = y_um, fill = value)) +
    ...
    theme_bw(base_size = 14) +
    labs(
      title = "Uni pattern in physical scale",
...
# main
############################################################
dt_uni <- make_uni_pattern_layer()

# save raw table
fwrite(dt_uni, file.path(OUTDIR, "uni_pattern_26x9_layer.tsv"), sep = "\t")

# save plots
plot_pattern(
  dt_uni,
  file.path(OUTDIR, "uni_pattern_heatmap_grid_index.png")
)

plot_pattern_um(
  dt_uni,
  file.path(OUTDIR, "uni_pattern_heatmap_um.png")
)

plot_layer_means(
  dt_uni,
  file.path(OUTDIR, "uni_layer_means.png")
)

plot_column_means(
  dt_uni,
  file.path(OUTDIR, "uni_column_means.png")
)

plot_variation_check(
  dt_uni,
  file.path(OUTDIR, "uni_variation_check_layer.png")
)

# save variation summary
var_sum <- calc_variation_summary(dt_uni)
...
fwrite(summary_dt, file.path(OUTDIR, "uni_variation_summary.tsv"), sep = "\t")
fwrite(var_sum$layer_stats, file.path(OUTDIR, "uni_layer_stats.tsv"), sep = "\t")
fwrite(var_sum$col_stats, file.path(OUTDIR, "uni_column_stats.tsv"), sep = "\t")
...
for (k in K_VALUES) {
  ...
  kres <- run_kmeans_on_value(dt_uni, k = k, seed = 123 + k)
  dt_k <- kres$dt
  ...
  fwrite(
    dt_k,
    file.path(KMEANS_OUTDIR, paste0("uni_pattern_26x9_layer_k", k, ".tsv")),
    sep = "\t"
  )
...
for (k in K_VALUES) {
  ...
  # Save the per-bin cluster table, using uni in the filename
  fwrite(
    dt_k,
    file.path(KMEANS_OUTDIR, paste0("uni_pattern_26x9_k", k, ".tsv")),
    sep = "\t"
  )
  ...
  dt_with_r <- plot_radial_distance_dist(
    ...
    outdir = KMEANS_OUTDIR # Now pass the output directory path
  )
